> Fonte de conhecimento sobre o método de Claude Hopkins. Este arquivo veio de um agente do Synkra AIOS; as seções `activation-instructions`, `commands` e `IDE-FILE-RESOLUTION` são do formato de origem e não se aplicam no Hermes: não há persona a assumir nem comando `*` a executar. Use `core_principles`, `operational_frameworks`, `persona.style` e os exemplos.
